...
namespace GlanMark
{
    public partial class frmSapInvoiceUpload : Form
    {
        ...
        public void ReadExcelFileAndInsertrecordInDB()
        {
            ...
            for (int ctr = 0; ctr < count; ctr++)
            {
                ...
                try
                {
                    ...
                    Batch = Convert.ToString(TableData.Rows[ctr]["Batch"]).Replace("'", "").Trim();
                    ...
                    if (TableData.Rows[ctr]["ZSTK"].ToString() != "")
                        ZSTK = Convert.ToString(TableData.Rows[ctr]["ZSTK"]);
                ...
                //if (Convert.ToString(IOMNO_STN) == "" | SoldToParty == "" | MaterialCode == "" | Batch == "")
                //{
                //    FlagStatus = true;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (bstatus == true)
            {
                ...
                {
                    var result = MessageBox.Show("Do you want ReUpload Today Records ? ", "SapInvoice", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        public void InsertRecordDB()
        {
            try
            {
                ...
                {
                    objCommand = new SqlCommand(@"insert into SAPInvoiceUpload (IOMNO,STNNumber,BillingDocument,SoldToParty,Name,Plnt,BillingDate,MaterialCode, Description,BilledQuantity,UpdateQuantity,Batch,Priority,ActualBilledQuantity,Subtotal1,Subtotal2,Subtotal3,Subtotal4,Subtotal5,Total,ZST1, ZEXDD,ZCESS,ZMR1,ZSTK,ZEXD,Mfgdate,ExpDate,NRV,LastModify,AlisCode) values (@IOMNO,@STNNumber,@BillingDocument,@SoldToParty,@Name,@Plnt,@BillingDate, @MaterialCode,@Description,@BilledQuantity,@UpdateQuantity,@Batch,@Priority,@ActualBilledQuantity,@Subtotal1,@Subtotal2,@Subtotal3,
@Subtotal4,@Subtotal5,@Total,@ZST1,@ZEXDD,@ZCESS,@ZMR1,@ZSTK,@ZEXD,@Mfgdate,@ExpDate,@NRV, '" + DateTime.Parse(uploaddate.Text, dateformat) + "', @AlisCode)", objConnection);
                }
                else
                {
                    objCommand = new SqlCommand(@"insert into SAPInvoiceUpload (IOMNO,STNNumber,BillingDocument,SoldToParty,Name,Plnt,BillingDate,MaterialCode, Description,BilledQuantity,UpdateQuantity,Batch,Priority,ActualBilledQuantity,Subtotal1,Subtotal2,Subtotal3,Subtotal4,Subtotal5,Total,ZST1, ZEXDD,ZCESS,ZMR1,ZSTK,ZEXD,Mfgdate,ExpDate,NRV,LastModify,AlisCode) values (@IOMNO,@STNNumber,@BillingDocument,@SoldToParty,@Name,@Plnt,@BillingDate, @MaterialCode,@Description,@BilledQuantity,@UpdateQuantity,@Batch,@Priority,@ActualBilledQuantity,@Subtotal1,@Subtotal2,@Subtotal3,
@Subtotal4,@Subtotal5,@Total,@ZST1,@ZEXDD,@ZCESS,@ZMR1,@ZSTK,@ZEXD,@Mfgdate,@ExpDate,@NRV, '" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "', @AlisCode)", objConnection);
                }
                ...
                objConn.Open();
                string Query1 = @"select * from SAPInvoiceUpload where (CONVERT(varchar, DATEPART(YYYY, LastModify) + DATEPART(MM, LastModify) + DATEPART(dd, LastModify))+CONVERT(varchar,IOMNO) + MaterialCode +Batch) = (CONVERT(varchar, DATEPART(YYYY, '" + lastmodifydatetime + "') + DATEPART(MM, '" + lastmodifydatetime + "') + DATEPART(dd, '" + lastmodifydatetime + "'))+ CONVERT(varchar," + IOMNO_STN + ") + '" + MaterialCode + "' +'" + Batch + "')";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var result = MessageBox.Show("Are you Sure Want To Do Integrity Test ? ", "SapInvoice", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                btnUpload.Visible = false;

                SAPInvoiceUploadIntegrityTest("SAPInvoiceUpload");
                BindSAPInvoice();
        ...
        public void SAPInvoiceUploadIntegrityTest(string Tablename)
        {
            ...
            //objcmd.CommandText = "SAP_IBIS_STN_Integrity";
            objcmd.CommandText = "SAPInvoice_IntegrityTest";
        ...
        public void BindSAPInvoice()
        {
            try
            {
                ...
                //sqlDataAdapter = objC.GetSqlDataAdapter("select * from SAPInvoiceUpload order by  1 desc");
                string dtFormat = CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern;
                if (dtFormat == "MM/dd/yyyy" || dtFormat == "M/d/yyyy")
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from SAPInvoiceUpload where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') ");
                }
                else
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from SAPInvoiceUpload where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') ");
                }
        ...
        public void LastModifydateofSAPInvoiceUpload()
        {
            try
            {
                ...
                dt = objCommon.GetDataTable("select top(1)* from SAPInvoiceUpload  order by 1 desc ");
        ...
        public string CheckTodayRecord(string dtDate)
        {
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                CommonFunction objCommon = new CommonFunction();
                dt = objCommon.GetDataTable("select COUNT(*) as NumberOfRecord from SAPInvoiceUpload where DATEPART(YYYY, LastModify) = DATEPART(YYYY, '" + dtSelectedDate + "') and DATEPART(MM, LastModify) = DATEPART(MM, '" + dtSelectedDate + "') and DATEPART(dd, LastModify) = DATEPART(dd, '" + dtSelectedDate + "') ");
        ...
        public string DeleteTodayRecord()
        {
            ...
            try
            {
                ...
                string querystring = "delete from SAPInvoiceUpload where ID in (select id from SAPInvoiceUpload where DATEPART(YYYY, LastModify) = DATEPART(YYYY, GETDATE()) and DATEPART(MM, LastModify) = DATEPART(MM, GETDATE()) and DATEPART(dd, LastModify) = DATEPART(dd, GETDATE()))";
        ...
        private void BtnUpdateorderitem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                if (IsCheckRStaus() != true)
                {
                    ...
                    objcmd.CommandText = "UpdateOrderItemBySAPInvoice";
                    objcmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool IsCheckRStaus()
        {
            ...
            objcmd.CommandText = "select * from SAPInvoiceUpload where DATEPART(YYYY, LastModify) = DATEPART(YYYY, '" + lastmodifydatetime2 + "') and DATEPART(MM, LastModify) = DATEPART(MM, '" + lastmodifydatetime2 + "') and DATEPART(dd, LastModify) = DATEPART(dd, '" + lastmodifydatetime2 + "')  and RStatus!='OK'";
        ...
        {
            CommonFunction objCF = new CommonFunction();
            objCF.fn_ExportToExcel("select *  from SAPInvoiceUpload where DATEPART(YYYY, LastModify) = DATEPART(YYYY, GETDATE()) and DATEPART(MM, LastModify) = DATEPART(MM, GETDATE()) and DATEPART(dd, LastModify) = DATEPART(dd, GETDATE()) ", "SAPInvoiceUpload", "SAPInvoiceUpload");
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objConnection = new SqlConnection(ConfigurationSettings.AppSettings[DBSessionUser.iYearId].ToString());
                objConnection.Open();
                string Query1 = "delete from SAPInvoiceUpload where RStatus != 'OK' and (CONVERT(varchar, DATEPART(YYYY, LastModify) + DATEPART(MM, LastModify) + DATEPART(dd, LastModify))) = (CONVERT(varchar, DATEPART(YYYY, @lastmodifydatetime) + DATEPART(MM, @lastmodifydatetime) + DATEPART(dd, @lastmodifydatetime)))";
        ...
        private void rtDuplicateRecord_TextChanged(object sender, EventArgs e)
        {
        ...
        private void btnSubmitDate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dtFormat == "MM/dd/yyyy" || dtFormat == "M/d/yyyy")
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from SAPInvoiceUpload where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') ");
                }
                else
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from SAPInvoiceUpload where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') ");
                }
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/btnSubmitDate_Click button \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
