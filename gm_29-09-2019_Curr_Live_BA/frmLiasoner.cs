...
namespace GlanMark
{
    public partial class frmLiasoner : Form
    {
        ...
        private void frmLiasoner_Load(object sender, EventArgs e)
        {
            bindGrid();
            bindSatus();
        }
        ...
        private void loadPage()
        {
            try
            {
                ...
                sqlDataAdapter = objC.GetSqlDataAdapter("SELECT TOP " + this.mintPageSize + " LiasonerId, LiasonerCode, LiasonerName, IsActive FROM LiasonerMaster WHERE LiasonerId NOT IN (SELECT TOP " + intSkip + " LiasonerId FROM LiasonerMaster)");
                ...
                dgvLiasonerMaster.DataSource = bindingSource;
        ...
        {
            objC = new CommonFunction();
            Result objres = objC.ExecuteScalarCnt("SELECT Rows FROM SYSINDEXES WHERE Id = OBJECT_ID('LiasonerMaster') AND IndId < 2");
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            objCF.fn_ExportToExcel("select LiasonerId as ID, LiasonerCode, LiasonerName, IsActive, LastModify from LiasonerMaster", "Liasoner Master", "LiasonerMaster");
        }
        ...
        private void Categorydgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex != -1)
                {
                    id = int.Parse(dgvLiasonerMaster.Rows[e.RowIndex].Cells["colLiasonerId"].Value.ToString());
                    if (e.ColumnIndex == dgvLiasonerMaster.Columns["colEdit"].Index)
                    {
                        lblStatus.Text = "";
                        txtLiasonerCode.Text = dgvLiasonerMaster[dgvLiasonerMaster.Columns["colCode"].Index, e.RowIndex].Value.ToString();
                        txtLiasonerName.Text = dgvLiasonerMaster[dgvLiasonerMaster.Columns["colName"].Index, e.RowIndex].Value.ToString();
                        cmbStatus.SelectedValue = dgvLiasonerMaster[dgvLiasonerMaster.Columns["colStatus"].Index, e.RowIndex].Value.ToString();
                    ...
                    else if (e.ColumnIndex == dgvLiasonerMaster.Columns["colDelete"].Index)
                    {
                        var result = MessageBox.Show("Do you want delete this Liasoner", "Delete Box", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (result == DialogResult.Yes)
                        {
                            objC = new CommonFunction();
                            Result res = objC.ExecuteDMLQuery("UPDATE LiasonerMaster SET IsActive = 'No' where LiasonerId='" + id + "'");
                            if (res.bStatus)
                            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bstatus == true)
                {
                    ...
                    {
                        string query = "INSERT INTO LiasonerMaster (LiasonerCode, LiasonerName, IsActive, LastModify) VALUES ('" + txtLiasonerCode.Text + "','" + txtLiasonerName.Text + "','" + cmbStatus.SelectedValue.ToString() + "', getdate())";
                        ...
                        if (objResult.bStatus)
                        {
                            ...
                            lblStatus.Text = "LiasonerId Master Inserted Sucessfully";
                            ClearForm();
                            bindGrid();
                        }
                        else
                    ...
                    {
                        string query = "UPDATE LiasonerMaster set LiasonerCode='" + txtLiasonerCode.Text + "',LiasonerName='" + txtLiasonerName.Text + "',IsActive='" + cmbStatus.SelectedValue.ToString() + "',LastModify=getdate() WHERE LiasonerId='" + lblHdnSource.Text + "'";
                        Result objResult = objCF.ExecuteDMLQuery(query);
                        if (objResult.bStatus)
                        {
                            lblStatus.ForeColor = System.Drawing.Color.FromArgb(128, 128, 255);
                            lblStatus.Text = "LiasonerId Master Updated Sucessfully";
                            lblHdnSource.Text = "0";
                            ClearForm();
                            bindGrid();
                        }
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/frmLiasonerMaster save button \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private bool validateForm()
        {
            ...
            if (txtLiasonerCode.Text == "")
            {
                errorProvider.SetError(txtLiasonerCode, "Liasoner Code");
                isValid = false;
            }

            if (txtLiasonerName.Text == "")
            {
                errorProvider.SetError(txtLiasonerName, "Liasoner Name");
                isValid = false;
            }
        ...
        {
            txtLiasonerCode.Text = "";
            txtLiasonerName.Text = "";
