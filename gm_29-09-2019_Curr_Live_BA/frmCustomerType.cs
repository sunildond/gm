...
namespace GlanMark
{
    public partial class frmCustomerType : Form
    {
        ...
        public void BindGrid()
        {
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/frmCustomerType Grid \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void loadPage()
        {
            try
            {
                ...
                sqlDataAdapter = objC.GetSqlDataAdapter("SELECT TOP " + this.mintPageSize + " ID, CustomerType, Description, IsActive from CustomerType WHERE ID NOT IN (SELECT TOP " + intSkip + " ID FROM CustomerType)");
                ...
                dgvCustomerType.DataSource = bindingSource;
        ...
        {
            objC = new CommonFunction();
            Result objres = objC.ExecuteScalarCnt("SELECT Rows FROM SYSINDEXES WHERE Id = OBJECT_ID('CustomerType') AND IndId < 2");
        ...
        private void dgvCustomerTypeMaster_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex != -1)
                {
                    id = int.Parse(dgvCustomerType.Rows[e.RowIndex].Cells["colId"].Value.ToString());

                    if (e.ColumnIndex == dgvCustomerType.Columns["colEdit"].Index)
                    {
                        //btnSave.Text = "Update";
                        txtCustomerType.Text = dgvCustomerType[dgvCustomerType.Columns["colCustomerType"].Index, e.RowIndex].Value.ToString();
                        txtDescription.Text = dgvCustomerType[dgvCustomerType.Columns["colDescription"].Index, e.RowIndex].Value.ToString();
                        cmbStatus.SelectedValue = dgvCustomerType[dgvCustomerType.Columns["colIsActive"].Index, e.RowIndex].Value.ToString();
                        lblID.Text = id.ToString();
                    }

                    if (e.ColumnIndex == dgvCustomerType.Columns["colDelete"].Index)
                    {
                        var result = MessageBox.Show("Do you want delete this CustomerType", "Delete Box", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (result == DialogResult.Yes)
                        {
                            objC = new CommonFunction();
                            Result res = objC.ExecuteDMLQuery("UPDATE CustomerType SET IsActive = 'No' where ID = '" + id + "'");
                            if (res.bStatus)
                            {
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/frmCustomerTypeMaster CellContentClick \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bstatus == true)
                {
                    ...
                    if (lblID.Text == "0")
                    {
                        string query = "INSERT INTO CustomerType (CustomerType, Description, IsActive, LastModify) VALUES ('" + txtCustomerType.Text + "','" + txtDescription.Text + "','" + cmbStatus.SelectedValue.ToString() + "', getdate())";
                        ...
                        if (objResult.bStatus)
                        {
                            ...
                            lblStatus.Text = "CustomerType Master Inserted Sucessfully";
                            ClearForm();
                            BindGrid();
                        }
                        else
                    ...
                    {
                        string query = "UPDATE CustomerType SET CustomerType = '" + txtCustomerType.Text + "', Description = '" + txtDescription.Text + "', IsActive = '" + cmbStatus.SelectedValue.ToString() + "', LastModify = getdate() where ID = " + lblID.Text;
                        //VALUES ('" + txtStamping.Text + "','" +  + "','" +  + "', )";
                        Result objResult = objCF.ExecuteDMLQuery(query);
                        if (objResult.bStatus)
                        {
                            lblStatus.ForeColor = System.Drawing.Color.FromArgb(128, 128, 255);
                            lblStatus.Text = "CustomerType Master Updated Sucessfully";
                            ClearForm();
                            BindGrid();
                        }
                        else
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/frmCustomerTypeMaster save button \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            {
                StreamWriter swLog = File.AppendText(strLogFileName);
                string strError = DateTime.Now.ToString() + "\n Main Page/frmCustomerType Master cancel button \n" + ex.ToString();
                swLog.WriteLine(strError);
                swLog.WriteLine();
                swLog.Close();
                MessageBox.Show(strError, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            errorCustomerType.Clear();
            bool isValid = true;
            if (txtCustomerType.Text == "")
            {
                errorCustomerType.SetError(txtCustomerType, "Code");
                isValid = false;
            }

            if (txtDescription.Text == "")
            {
                errorCustomerType.SetError(txtDescription, "Zone");
                isValid = false;
            }
        ...
        {
            CommonFunction objCF = new CommonFunction();
            objCF.fn_ExportToExcel("select * from CustomerType", "Customer Type", "CustomerType");
        }
