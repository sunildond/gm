...
namespace GlanMark
{
    public partial class frmIBISBillingThrughSTN : Form
    {
       ...
       // string strLogFileName = "LOG/LogRecord.txt";
        ...
        public frmIBISBillingThrughSTN()
        {
            InitializeComponent();
        }
        ...
        public void BindIbIS()
        {
            try
            {
                ...
                //sqlDataAdapter = objC.GetSqlDataAdapter("select * from IBISBillingUpdate");
                string dtFormat = CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern;
                if (dtFormat == "MM/dd/yyyy" || dtFormat == "M/d/yyyy")
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from IBISBillingUpdate where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(uploaddate.Text, dateformat) + "') ");
                }
                else
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from IBISBillingUpdate where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(uploaddate.Text).ToString(_Strdateformat) + "') ");
                }
            ...
            CommonFunction objComm = new CommonFunction();
                ...
                try
                {
                    ...
                    if (TableData.Rows[ctr]["IOMNO_STN"].ToString() != "")
                        ...
                        //IOMNO = int.Parse(TableData.Rows[ctr]["IOMNO_STN"]);
        ...
        public string CheckTodayRecord(string dtDate)
        {
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                CommonFunction objCommon = new CommonFunction();
                dt = objCommon.GetDataTable("select COUNT(*) as NumberOfRecord from IBISBillingUpdate where DATEPART(YYYY, LastModify) = DATEPART(YYYY, '" + dtSelectedDate + "') and DATEPART(MM, LastModify) = DATEPART(MM, '" + dtSelectedDate + "') and DATEPART(dd, LastModify) = DATEPART(dd, '" + dtSelectedDate + "') ");
        ...
        public void InsertRecordDB()
        {
            try
            {
                ...
                objConn.Open();
                string Query1 = @"select * from IBISBillingUpdate where (CONVERT(varchar, DATEPART(YYYY, LastModify) + DATEPART(MM, LastModify) + DATEPART(dd, LastModify))+CONVERT(varchar,IOMNO) + MaterialCode +Batch) = (CONVERT(varchar, DATEPART(YYYY, '" + lastmodifydatetime + "') + DATEPART(MM, '" + lastmodifydatetime + "') + DATEPART(dd, '" + lastmodifydatetime + "'))+ CONVERT(varchar," + IOMNO_STN + ") + '" + MaterialCode + "' +'" + Batch + "')";
        ...
        private void btnpendingorder_Click_1(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                IBISIBISIntegrityTest("IBISBillingUpdate_Integrity_Test_ThrughSTN");
        ...
        public bool IsCheckRStaus()
        {
            ...
            objcmd.CommandText = "select * from IBISBillingUpdate where DATEPART(YYYY, LastModify) = DATEPART(YYYY, '" + lastmodifydatetime2 + "') and DATEPART(MM, LastModify) = DATEPART(MM, '" + lastmodifydatetime2 + "') and DATEPART(dd, LastModify) = DATEPART(dd, '" + lastmodifydatetime2 + "')  and RStatus!='OK'";
        ...
        private void BtnUpdateorderitem_Click_1(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                if (IsCheckRStaus() != true)
                {
                    ...
                    objcmd.CommandText = "UpdateOrderItemByIBISthrughSTN";
                    objcmd.CommandType = CommandType.StoredProcedure;
                    //string lastmodifydatetime2 = DateTime.Parse(uploaddate.Text, dateformat).ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objConnection = new SqlConnection(ConfigurationSettings.AppSettings[DBSessionUser.iYearId].ToString());
                objConnection.Open();
                string Query1 = "delete from IBISBillingUpdate where RStatus != 'OK' and (CONVERT(varchar, DATEPART(YYYY, LastModify) + DATEPART(MM, LastModify) + DATEPART(dd, LastModify))) = (CONVERT(varchar, DATEPART(YYYY, @lastmodifydatetime) + DATEPART(MM, @lastmodifydatetime) + DATEPART(dd, @lastmodifydatetime)))";
        ...
        private void btnSubmitDate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dtFormat == "MM/dd/yyyy" || dtFormat == "M/d/yyyy")
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from IBISBillingUpdate where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(dtpGridBind.Text, dateformat) + "') ");
                }
                else
                {
                    sqlDataAdapter = objC.GetSqlDataAdapter(@"select * from IBISBillingUpdate where DATEPART(dd,LastModify) = DATEPART(dd,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') and DATEPART(MM,LastModify) = DATEPART(MM,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') and DATEPART(yyyy,LastModify) = DATEPART(yyyy,'" + DateTime.Parse(dtpGridBind.Text).ToString(_Strdateformat) + "') ");
                }
