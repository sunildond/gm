...
namespace GlanMark
{
    public partial class Orderdetail : Form
    {
        ...
        public Orderdetail(int Orderheaderid, int i)
        {
            InitializeComponent();
            flag = i;
        ...
        private void BindOrder(int Orderheaderid)
        {
            try
            {
                ...
                sqlDataAdapter = objC.GetSqlDataAdapter(@"select (case when OrderHeader.Institution is not null then 
(select Name from InstitutionMaster where InstitutionMaster.Code = OrderHeader.Institution) end) InstitutionName,

(case when OrderHeader.SubInstitution is not null then 
(select Name from SubInstitutionMaster where SubInstitutionMaster.Code = OrderHeader.SubInstitution) end) SubInstitutionName,

(case when OrderHeader.OrderType is not null then 
(select OrderTypeName from OrderTypeMaster where OrderTypeMaster.ID = OrderHeader.OrderType) end) OrderTypeName,

(case when OrderHeader.ShelfLife is not null then 
(select ShelfLifeName from ShelfLifeMaster where ShelfLifeMaster.ShelfLifeId = OrderHeader.ShelfLife) end) ShelfLifeName,

(case when OrderHeader.StampingID is not null then 
(select Name + ' (' + Code + ')' from StampingMaster where StampingMaster.StampingId = OrderHeader.StampingID) end) StampingName,
 * 
from OrderHeader where OrderHeader.OrderHeaderId ='" + Orderheaderid + "'");

                dataTable = new DataTable();
                sqlDataAdapter.Fill(dataTable);
                lblLocalPlace.Text = dataTable.Rows[0]["LocalPlace"].ToString();
                ...
                if (dataTable.Rows[0]["DocFile1"].ToString() != "")
                    ...
                    //lnkDocFile.Text = ConfigurationSettings.AppSettings["ServerPath"].ToString() + dataTable.Rows[0]["DocFile1"].ToString();
                else
                ...
                //lblDocRequired.Text = dataTable.Rows[0]["DocumentReq"].ToString();
        ...
        private void BindProductSchedule(int IOMNo)
        {
            try
            {
                ...
                sqlDataAdapter = objC.GetSqlDataAdapter("select IOMID,IOMNo,ProductCode,AlisCode,(case when OrderItem.AlisCode is not null then (select AliasName from ProductMaster where OrderItem.ProductId = ProductMaster.ProductId) end) AliasName,InstRate, Quantity,BillingRate,Commision,MRP,ProdValue,Tax,TaxValue,ProductName from OrderItem where IOMNo = " + IOMNo);
        ...
        private void BindOrderItemSchedule(int IOMNo, string prodCode)
        {
            try
            {
                ...
                //lblSchOrderID.Text = IOMNo.ToString();
                objC = new CommonFunction();
                sqlDataAdapter = objC.GetSqlDataAdapter("select IOMNo,MaterialCode,OrderQuantity,ScheduleDate,DeliveryDate from ScheduleDetail where IOMNo = '" + IOMNo + "' and MaterialCode='" + prodCode + "'");
        ...
        {
            try  //25003572-25006223-9819706223-sbi
            {    //24133145-24132007-bom
                int chkstatus = 0;
                ...
                {
                    chkstatus = 1;
                    query = "UPDATE OrderHeader set Authorised='" + chkstatus + "', OrderAuthoDate = getdate() WHERE OrderHeaderId='" + lblOrderHeaderId.Text + "'";
                    msgbox = "Order Authorised Successfully";
                }
                else
                {
                    query = "UPDATE OrderHeader set Authorised='" + chkstatus + "', OrderAuthoDate = NULL WHERE OrderHeaderId='" + lblOrderHeaderId.Text + "'";
                    msgbox = "Order UnAuthorised Successfully";
                }
                ...
                {
                    if (MessageBox.Show(msgbox) == DialogResult.OK)
                    {
        ...
        private void chkAuthorize_CheckedChanged(object sender, EventArgs e)
        {
