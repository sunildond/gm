...
namespace GlanMark
{
    public partial class filterSTNDispatch : Form
    {
        ...
        public filterSTNDispatch()
        {
            InitializeComponent();

            CommonFunction objCommon = new CommonFunction();
            CommDT = objCommon.GetDataTable(ConfigurationSettings.AppSettings["STNDispatch"].ToString());
            bindIOMNo();
            BindPartyCode();
            BindDSMZSM();
            BindLocationCode();
        ...
        protected void execQuery(string strQuery)
        {
            try
            {
                ...
                sqlDataAdapter = objC.GetSqlDataAdapter(strQuery.ToString());  //objC.GetSqlDataAdapter(ConfigurationSettings.AppSettings["STNDispatch"].ToString());
                txtQuery.Text = strQuery.ToString();    //ConfigurationSettings.AppSettings["STNDispatch"].ToString();
            ...
            {
                MessageBox.Show("STN Dispatch not get Executed!");
            }
        }
        ...
        private void BindSTNLocationCode()
        {
            try
            {
                cmbSTNLocation.AutoCompleteSource = AutoCompleteSource.ListItems;
                cmbSTNLocation.AutoCompleteMode = AutoCompleteMode.Suggest;
                DataTable uniqueCols = CommDT.DefaultView.ToTable(true, "STNLocation");
                cmbSTNLocation.DataSource = uniqueCols;
                cmbSTNLocation.DisplayMember = "STNLocation";
                cmbSTNLocation.ValueMember = "STNLocation";
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            try
            {
                string Query = "select * from ( " + ConfigurationSettings.AppSettings["STNDispatch"].ToString() + " ) as a ";
                ...
                if (chkSTNLocation.Checked)
                {
                    if (cmbSTNLocation.SelectedValue.ToString() != "")
                    {
                        if (strWhere.ToString().Trim() != "")
                            strWhere.Append(" and ");
                        strWhere.Append(" a.STNLocation = '" + cmbSTNLocation.SelectedValue.ToString() + "' ");
                    }
                }
                ...
                if (chkAuthDate.Checked)
                {
                    if (dtpFromAuthDate.Checked == true && dtpToAuthDate.Checked == true)
                    {
                        ...
                        if (dtFormat == "MM/dd/yyyy" || dtFormat == "M/d/yyyy")
                        {
                            strWhere.Append(" a.AuthDate between '" + DateTime.Parse(dtpFromAuthDate.Text, dateformat) + "' and '" + DateTime.Parse(dtpToAuthDate.Text, dateformat) + "' ");
                        }
                        else
                        {
                            strWhere.Append(" a.AuthDate between '" + DateTime.Parse(dtpFromAuthDate.Text).ToString(_dateformat) + "' and '" + DateTime.Parse(dtpToAuthDate.Text).ToString(_dateformat) + "' ");
                        }
                    }
                }
        ...
        {
            execQuery(ConfigurationSettings.AppSettings["STNDispatch"].ToString());
        }
